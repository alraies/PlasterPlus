...
class OffersController extends Controller
{
    ...
    {
        // if(!Helpers::get_restaurant_data()->offer_section)
        // {
    ...
    public function store(Request $request)
    {
        ...
        $offer = new offer;
        $offer->name = $request->name[array_search('en', $request->lang)];
        ...
        $offer->category_id = $request->sub_category_id?$request->sub_category_id:$request->category_id;
        $offer->category_ids = json_encode($category);
        $offer->description = $request->description[array_search('en', $request->lang)];
        ...
        $offer->choice_options = json_encode($choice_options);
        $variations = [];
        $options = [];
        ...
        //combinations end
        $offer->variations = json_encode($variations);
        $offer->price = $request->price;

        $offer->image = Helpers::upload('product/', 'png', $request->file('image'));
        // $offer->available_time_starts = $request->available_time_starts;
        // $offer->available_time_ends = $request->available_time_ends;
        $offer->discount = $request->discount_type == 'amount' ? $request->discount : $request->discount;
        $offer->discount_type = $request->discount_type;
        $offer->attributes = $request->has('attribute_id') ? json_encode($request->attribute_id) : json_encode([]);
        $offer->add_ons = $request->has('addon_ids') ? json_encode($request->addon_ids) : json_encode([]);
        $offer->restaurant_id = Helpers::get_restaurant_id();
        $offer->save();
    ...
    public function view($id)
    {
        ...
        $reviews=Review::where(['offer_id'=>$id])->latest()->paginate(config('default_pagination'));
    ...
    public function edit($id)
    {
        ...
        $product = offer::withoutGlobalScope('translate')->findOrFail($id);
    ...
    public function search(Request $request){
        ...
        $offers=offer::where(function ($q) use ($key) {
    ...
    public function bulk_export_data(Request $request)
    {
        ...
        ->get();
        return (new FastExcel(ProductLogic::format_export_foods($products)))->download('offers.xlsx');
    }
}
